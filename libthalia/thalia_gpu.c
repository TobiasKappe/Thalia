...
void thalia_gpu_handle_dma(ThaliaGB* gb, guint8 addr_msb)
{
	...
	for(i = 0; i < 0xA0; i++)
		thalia_mmu_write_byte(gb, 0xFE00 + i, thalia_mmu_read_byte(gb, addr + i));
	gb->cycles += 0xA0; // TODO: Figure out clock penalty
}

// Check for a scanline coincidence interrupt using the current line and compare
// line I/O registers and schedule the interrupt if one has occured.
static void thalia_gpu_check_scanline_interrupt(ThaliaGB* gb)
{
	if(gb->mmu->ram_io.unpacked.int_scanline_coincidence &&
	   gb->mmu->ram_io.unpacked.line_cur == gb->mmu->ram_io.unpacked.line_cmp) {
	...
	if(gb->mmu->ram_io.unpacked.lcd_bg_display ||
	   gb->mmu->ram_io.unpacked.lcd_wd_display) {
		...
		for(screen_xpos = 0; screen_xpos < THALIA_GPU_SCREEN_WIDTH; screen_xpos++) {
			...
			// If we're inside the window, we need to render from a different tilemap.
			...
			// Get the tile from the tileset as indicated in the lcd control register.
			if(gb->mmu->ram_io.unpacked.lcd_tile_data) {
				if(tile_no < 0x80)
					tile = &gb->mmu->ram_gpu.unpacked.tileset_1[tile_no];
				else tile = &gb->mmu->ram_gpu.unpacked.tileset_s[tile_no - 0x80];
			} else {
				// The tile number is signed when this set is used.
				gint8 tile_no_signed = (gint8)tile_no;
				if(tile_no_signed < 0)
					tile = &gb->mmu->ram_gpu.unpacked.tileset_s[0x80+tile_no_signed];
				else tile = &gb->mmu->ram_gpu.unpacked.tileset_0[tile_no];
			}

			// Now that we have the right tile, get the color code at this position.
			guint8 code = 0;
 ...
 guchar* pixel_base, gint n_channels)
{
	...
	for(sprite_index = THALIA_GPU_N_SPRITES-1; sprite_index >= 0 &&
	  ...
	  // First grab the sprite object at this location from OAM.
		...
		else palette = gb->mmu->ram_io.unpacked.pal_obj0;

		// Grab the tile to be rendered from.
		if(sprite.tileno < 0x80)
			tile = &gb->mmu->ram_gpu.unpacked.tileset_1[sprite.tileno];
		else tile = &gb->mmu->ram_gpu.unpacked.tileset_s[sprite.tileno-0x80];
		...
		for(tile_xoff = 0; tile_xoff < 8; tile_xoff++) {
			...
			// Code zero means transparent, so we don't render that particular pixel.
			// Also, pixels of non-priority sprites are only rendered if the current
			// color at that position is white.
...
static void thalia_gpu_vblank(ThaliaGB* gb)
{
	...
	if(gb->mmu->ram_io.unpacked.line_cur >= THALIA_GPU_SCREEN_HEIGHT_EXTRA) {
		// If we're at the end of the extra lines, go back to the top of the screen.
		gb->mmu->ram_io.unpacked.line_cur = 0;
		gb->mmu->ram_io.unpacked.gpu_mode = THALIA_GPU_MODE_SCAN_OAM;
		thalia_gpu_check_status_interrupt(gb);

		if(gb->gpu.periods - gb->gpu.last_change < 2) {
			// Signal the consuming code that the screen may now be rendered. Note
			// that we can only continue once the consuming code relinquishes its
			// lock. This is done to prevent the emulation thread from continuing to
			// render, causing screen tearing.
			thalia_gpu_unlock();
...
void thalia_gpu_step(ThaliaGB* gb)
{
	...
	// 'left' holds the cycles that the GPU has yet to emulate. If there's enough
	// time accumulated to reach the end of a mode, kick off a relevant function.
