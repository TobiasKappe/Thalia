...
guint8 thalia_alu_add_carry(ThaliaGB* gb, guint8 a, guint8 b)
{
	...
	// We cannot pass control to thalia_alu_add because the flags are calculated
	// differently with regard to overflow
...
// Adjusts the given operand for binary-coded decimal representation on the ALU.
...
guint8 thalia_alu_update_flags_shift(ThaliaGB* gb, guint8 res, gboolean leaving)
{
	gb->reg.named.flag_zero = res == 0;
	gb->reg.named.flag_operation = FALSE;
	gb->reg.named.flag_halfcarry = FALSE;
...
guint8 thalia_alu_sra(ThaliaGB* gb, guint8 a)
{
	...
	// We can utilize this by casting to a signed type and shifting. This is the
	// case for GCC and clang on x86-ish architectures.
