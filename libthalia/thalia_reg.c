...
guint8 thalia_reg_read_single(ThaliaGB* gb, const thalia_regname_single_t reg)
{
    ...
    case THALIA_REG_IHL:
        g_debug(
            "(0x%04X) READ (HL): 0x%02X", gb->pc,
            thalia_mmu_read_word(
                gb,
                thalia_reg_read_word(gb, THALIA_REG_HL, TRUE)
            )
        );
        break;
    case THALIA_REG_A:
    ...
    if(reg == THALIA_REG_IHL) {
        ...
        return gb->reg.indexed[reg];
}

// Reads word register 'reg'. If 'type' is TRUE, the SP's index is read as AF.
                             ...
                             const guint16 value, gboolean type)
{
    ...
    case THALIA_REG_SP:
        if(!type)
            ...
            g_debug("(0x%04X) WRITE AF: 0x%04X", gb->pc, value);
        break;
    }
#endif
